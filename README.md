...
and submit an email request inside the website. The server sends the request to
the support Gmail account and CCs the guest. The host confirms availability,
price, and the private arrival details after accepting a reservation.
...
- The booking calendar shows the next twelve months, supports date selection,
  unavailable-night blocking, keyboard-friendly interactions, a sticky
  shortcut, and a standard-rate price estimate.
- Airbnb remains the current availability source of truth. Its private iCalendar
  feed is read at runtime by the full application, re-checked when a request is
  submitted, and synchronized every five minutes for the NAS mirror.
...
- Twelve-month date picker with arrival/departure selection
...
- Published Monday–Thursday, Friday–Sunday, long-weekend, and cleaning-fee rates
- In-site request dialog for guest email, selected dates, guest count, and notes
- Server-sent Gmail message to the support inbox with the guest CCed
...
- Five-minute NAS Airbnb availability synchronization
...
| Booking requests | Gmail API with server-side OAuth credentials | Sends to the support inbox and CCs the guest without opening external software. |
...
Private Airbnb iCalendar URL
            ...
            |
            +--> NAS sync container (every 5m)
                    --> static-site/airbnb-availability.json
                    --> nginx static calendar

Guest chooses dates --> in-site request form
                    --> live Airbnb re-check
                    --> Gmail API sends to support + CC guest
...
app/
  ...
  api/booking-request/route.ts in-site request validation and Gmail delivery
...
scripts/
  ...
  booking-request-server.mjs   NAS request API and Gmail delivery
...
hosting platform. `.env*`, `nas/airbnb-calendar.env`,
`nas/booking-request.env`, monitor state, and monitor logs are ignored.
...
| `BOOKING_SUPPORT_EMAIL` | Sender/recipient support address; defaults to LakefrontSerenitySupport@gmail.com | Yes for request delivery |
| `GMAIL_CLIENT_ID` | Google OAuth client used by the server | Yes for request delivery |
| `GMAIL_CLIENT_SECRET` | Google OAuth client secret used by the server | Yes for request delivery |
| `GMAIL_REFRESH_TOKEN` | Gmail authorization with `gmail.send` access | Yes for request delivery |
...
Never commit the Airbnb feed URL, Gmail OAuth credentials, Stripe keys, access
codes, or the exact arrival address. The `.ics` URL functions like a private
credential because anyone with it can read booking timing.
...
Configure `AIRBNB_ICAL_URL`, `BOOKING_SUPPORT_EMAIL`, `GMAIL_CLIENT_ID`,
`GMAIL_CLIENT_SECRET`, and `GMAIL_REFRESH_TOKEN` as server-side runtime values
in the hosting platform. Do not expose them through public/client-prefixed
environment values.
...
The NAS requires uncommitted `nas/airbnb-calendar.env` and
`nas/booking-request.env` files:

```text
AIRBNB_ICAL_URL=https://www.airbnb.com/calendar/ical/...
```

Copy `nas/booking-request.env.example` to `nas/booking-request.env` and add the
Gmail OAuth values. The sync service updates
`static-site/airbnb-availability.json` every five minutes. Run
`scripts/check-airbnb-calendar.sh` every fifteen minutes from the host to detect
a stopped container or a feed older than thirty minutes. Outbound mail must be
configured in TrueNAS for monitor alerts to be delivered.

## Maintenance handoff

This section is the operating brief for the next developer or maintainer.
...
3. **Do not imply instant booking or payment.** The live CTA submits an in-site
   request which the server emails to the host and guest. The host confirms
   availability and pricing manually.
4. **Do not advertise a tax-free or fee-free stay.** Applicable licensing,
   ...
   Conduct still apply.
5. **Keep secrets server-side.** Never place `AIRBNB_ICAL_URL`, Gmail OAuth
   credentials, Stripe secrets, door codes, or private guest information in
   source, generated JSON, or client-side bundles.
