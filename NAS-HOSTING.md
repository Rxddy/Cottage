...
- Request API container: `lakefront-booking-request-api`
- Availability container: `lakefront-airbnb-calendar-sync`
...
The `lakefront-airbnb-calendar-sync` container refreshes the read-only Airbnb calendar every five minutes. The request API performs another live Airbnb check before it sends a guest request. The host-side `scripts/check-airbnb-calendar.sh` check should run every 15 minutes. It alerts the configured operators when the container is down or the feed is more than 30 minutes stale, then sends a recovery message when it returns to normal.
...
## In-site request email

Create `nas/booking-request.env` from
`nas/booking-request.env.example` and supply the Gmail OAuth client ID, client
secret, refresh token, and support email. This file is ignored by Git. The nginx
container proxies `/api/booking-request` to the private request API container;
guests stay on the website and do not need an email application.
